...
function budoMiddleware (entryMiddleware, opts) {
  ...
  staticPaths.forEach(function (pathDef) {
    var staticOpts = xtend({
      cacheControl: false
    }, opts.staticOptions)
    var dir = pathDef instanceof Array ? pathDef[1] : pathDef
    var staticHandler = serveStatic(dir, staticOpts)

    if (pathDef instanceof Array) {
      handler.mount(pathDef[0], function (req, res, next) {
