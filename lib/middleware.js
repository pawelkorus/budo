...
function budoMiddleware (entryMiddleware, opts) {
  ...
  staticPaths.map(mapDirectoryEntry).forEach(function (mapping) {
    var staticOpts = xtend({
      cacheControl: false
    }, opts.staticOptions)
    var staticHandler = serveStatic(mapping.from, staticOpts)
    if (mapping.from === mapping.url) {
      handler.use(function (req, res, next) {
        logHandler.type = 'static'
        staticHandler(req, res, next)
      })
    } else {
      handler.mount(mapping.url, function (req, res, next) {
        logHandler.type = 'static'
        staticHandler(req, res, next)
      })
    }
  ...
  function mapDirectoryEntry (mapping) {
    var pathFrom, pathUrl, parts
    ...
    if (parts.length > 1 && parts[1].length > 0) {
      ...
      if (pathFrom === '.') {
        pathFrom = resolve.sync('.', { basedir: process.cwd() })
      }
    } else {
      pathFrom = mapping
      pathUrl = mapping
